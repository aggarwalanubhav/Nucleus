...
{
	/*s1ap information*/
	g_s3_cfg.local_egtp_ip = get_ip_scalar("s3.egtp_local_addr");
	if(-1 == g_s3_cfg.local_egtp_ip) return -1;
	g_s3_cfg.egtp_def_port = get_int_scalar("s3.egtp_default_port");
	if(-1 == g_s3_cfg.egtp_def_port) return -1;
	g_s3_cfg.target_ip = get_ip_scalar("s3.target_ip");
	if(-1 == g_s3_cfg.local_egtp_ip) return -1;
	...
	/*struct local_config config_addr[] = 
	{
		{ 
		  .name = "s3.sgw_addr",
		  .addr = &g_s3_cfg.sgw_ip,
		},
		{ 
		  .name = "s3.pgw_addr",
		  .addr = &g_s3_cfg.pgw_ip,
		}
	};
